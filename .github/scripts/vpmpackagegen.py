...
INDEX_TEMPLATE = {
    "name": "Minetake Basis VPM Repository",
    "id": "net.minetake.basis.vpm",
    "url": "https://basis.minetake.net/vpm-repo.json",
    "author": "minetake",
    "packages": {},
}
...
def parse_args():
    ...
    build.add_argument("--packages", default="", metavar="LIST",
                       help="Comma-separated package folder names to build "
                            "(default: all entries in vpm-packages.txt).")
...
def filter_packages(manifest_packages, packages_filter):
    """Return manifest entries limited to packages_filter when non-empty."""
    if not packages_filter:
        return manifest_packages
    allowed = {p.strip() for p in packages_filter.split(",") if p.strip()}
    if not allowed:
        sys.exit("error: --packages was provided but parsed to an empty list")
    manifest_set = set(manifest_packages)
    unknown = sorted(allowed - manifest_set)
    if unknown:
        sys.exit(f"error: package(s) not listed in {MANIFEST.name}: {', '.join(unknown)}")
    selected = [p for p in manifest_packages if p in allowed]
    if not selected:
        sys.exit(f"error: no packages selected from filter: {packages_filter!r}")
    return selected
...
def cmd_build(args):
    ...
    manifest_packages = read_manifest(MANIFEST)
    packages = filter_packages(manifest_packages, args.packages)
    ...
    for pkg in packages:
        ...
        derived = compute_vpm_dependencies(pkg, manifest_packages, by_guid, by_name, by_dll, args.dev_suffix)
        if derived:
            merged = dict(derived)
        ...
        # Rewrite deps that point at known hub packages so --dev-suffix versions resolve.
